...
class MinimalSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        if freq not in latency_lists:
            print(20*'-')
            print(f"Messages are being sent with new Frequency: {freq}. New key in latency_lists")
            print(20*'-')
            latency_lists[freq] = []  # Initialize list if category is encountered for the first time
        if freq not in packet_sequences:
            print(20*'-')
            print(f"Messages are being sent with new Frequency: {freq}. New key in packet_sequences")
            print(20*'-')
            packet_sequences[freq] = []  # Initialize list if category is encountered for the first time
        latency_lists[freq].append(time_diff_nsec)
        packet_sequences[freq].append(msg.packet_id)
...
def check_out_of_sequence(packet_sequences):
    for frequency, sequence in packet_sequences.items():
        sorted_sequence = sorted(sequence)
        if sequence != sorted_sequence:
            print(f"Out of sequence messages detected in category {frequency}:")
            print(f"Original sequence: {sequence}")
            print(f"Sorted sequence: {sorted_sequence}")
            # You can implement further actions here, such as logging or handling the out-of-sequence messages
...
def signal_handler(sig, frame):
    ...
    check_out_of_sequence(packet_sequences)
