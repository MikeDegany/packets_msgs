...
def filter_outliers(latency_lists, threshold=0.2):
    """
    Filter out time values larger than the specified threshold from the latency_lists dictionary.

    Args:
        latency_lists (dict): Dictionary containing lists of time values.
        threshold (float): Threshold value for filtering outliers. Default is 0.2 seconds.

    Returns:
        dict: Filtered latency_lists dictionary.
    """
...
def signal_handler(sig, frame):
    ...
    for category, times in latency_lists_filtered.items():
        ...
        print(40*'-')
        print(40*'-')
        print(f'Still Outlier Detecte {category}:  {[time for time in times if time <= 0.2]}')
        print(40*'-')
        print(40*'-')
        print(40*'-')

    print("Ctrl+C pressed. Exiting...")
    print("Latency lists:", latency_lists_filtered)
    statistics = calculate_statistics(latency_lists_filtered)
    print("Statistics of latency lists:", statistics)
    generate_histogram('/home/UNT/md0708/plots/hists', latency_lists_filtered)
    plot_boxplot('/home/UNT/md0708/plots/boxplot', latency_lists_filtered)  # Call plot_boxplot function here
    plot_jitter_profiles('/home/UNT/md0708/plots/plots', latency_lists_filtered)
    packet_loss_rates = calculate_packet_loss(latency_lists_filtered)
